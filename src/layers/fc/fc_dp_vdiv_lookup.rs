...
pub struct FCDPVarDivLookupChip<F: PrimeField> {
  pub _marker: PhantomData<F>,
  pub config: FullyConnectedConfig,
}

impl<F: PrimeField> Layer<F> for FCDPVarDivLookupChip<F> {
  fn forward(
    &self,
  ...
  fn num_rows(&self, layer_config: &crate::layers::layer::LayerConfig, num_cols: i64) -> i64 {
    ...
    let mut num_rows =
      <FCDPVarDivLookupChip<F> as Layer<F>>::num_rows_dot_acc(inp_shape[1] as i64, num_cols)
...
impl<F: PrimeField> GadgetConsumer for FCDPVarDivLookupChip<F> {
