...
def get_implementations():
  # line 308 in model.rs
  layer_type_to_impl_idx = {
    "AveragePool2D": [0],
    "Add": [0],
    "BatchMatMul": [0],
    "Broadcast": [0],
  ...
  return layer_type_to_impl_idx

def create_logical_implementations(known_layers):
  logical_implementations = []
  specific_layer_types = [layer_type for layer_type in known_layers]
  specific_impl_indices = [get_implementations()[layer_type] for layer_type in known_layers]
  for p_idces in product(*specific_impl_indices):
    logical_implementation = dict()
    for layer_type, p_idce in zip(specific_layer_types, p_idces):
      logical_implementation[layer_type] = p_idce
    logical_implementations.append(logical_implementation)
  return logical_implementations
...
class Converter:
  def __init__(
    ...
    self.init_known_layers = False
    self.known_layers = None
    self.next_implementations = None
  
  def get_implementation(self, layer_type):
    if self.next_implementations is not None:
      return self.next_implementations[0][layer_type]
    return get_implementations()[layer_type][0]

  def set_next_implementations(self, next_implementations):
    self.next_implementations = next_implementations
  ...
  def to_dict(self, start_layer, end_layer):
    ...
    for op_idx in range(graph.OperatorsLength()):
      ...
      implementation_idx = self.get_implementation(layer_type)
      layers.append({
        ...
        'params': params,
        'mask': mask,
        'implementation': implementation_idx, # for line 379 in model.rs
      })
      layer_types.add(layer_type)
    print(layers)
    print()

    if not self.init_known_layers:
      self.known_layers = layer_types
      self.init_known_layers = True
...
def main():
  ...
  parser.add_argument('--config_output_dir', type=str, required=True)
  ...
  while keep_gen_logical_layout:
    ...
    with open(os.path.join(args.model_output_dir, "model_{}.msgpack".format(counter)), 'wb') as f:
        f.write(model_packed)
    with open(os.path.join(args.config_output_dir, "config_{}.msgpack".format(counter)), 'wb') as f:
        f.write(config_packed)
    
    if converter.next_implementations is None:
      next_implementations = create_logical_implementations(converter.known_layers)
      converter.set_next_implementations(next_implementations)
    next_implementations.pop(0) # because next_implementations[0] is used in this round

    keep_gen_logical_layout = converter.next_implementations != []
    counter+=1
